...
export class CompraService {
  ...
  getHistorialCompraPorPagina(
    ...
    return this.http.get<any>(
      `${this._urlApi}/historialCompra?page=${page}&fecha_inicial=${fecha_inicial}&fecha_final=${fecha_final}&hora_inicial=${hora_inicial}&hora_final=${hora_final}&estado=${estado}&size=${perPage}&search=${search}&typeReport=${typeReport}&tipo_factura_electronica=${tipo_factura}&bodega=${bodega}`,
      this.headers
    );
  }

  anularFacturaCompra(factura_id: any, motivo_anulacion: any) {
    var body = [];
    body.push({
      factura: factura_id,
    ...
    ////console.log(body);
    return this.http.post<any>(
      `${this._urlApi}/anularFacturaCompra`,
      body,
      this.headers
    );
  }
