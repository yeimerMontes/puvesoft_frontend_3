...
export class LoginComponent {
  ...
  login() {
    ...
    this.form.controls.type.setValue(this.sitioWeb === 'www.conectapos.info');
    ...
    this.authService.login(this.form.value).subscribe(
      (resp) => {
        ...
        localStorage.setItem(
          btoa('token'),
          encodeURIComponent(resp.access_token)
        );
        localStorage.setItem(btoa('rol'), btoa(encodeURIComponent(resp.rol)));
        localStorage.setItem(
          btoa('nombre'),
          btoa(encodeURIComponent(resp.nombre))
        );
