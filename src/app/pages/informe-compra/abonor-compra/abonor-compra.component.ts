...
@Component({
  selector: 'app-abonor-compra',
  templateUrl: './abonor-compra.component.html',
  styleUrls: ['./abonor-compra.component.css', '../../../css/modulo.css'],
})
export class AbonorCompraComponent implements OnInit {
  @Input() id: number;
  ...
  onConfirm(title: string, mensaje: string, functionConfirm, item = undefined) {
    ...
    Swal.fire({
      ...
      title: title,
      text: '' + mensaje + ' !',
      confirmButtonText: '<i class="fa fa-thumbs-up"></i> Sí, continuar',
  ...
  ngOnInit() {
    ...
    this.factura_compra_id = this.id;
    // this.factura_compra_id = this.activatedRoute.snapshot.paramMap.get('facturaId');
  ...
  abonar() {
    ...
    if (this.form.get('valor').value <= 0) {
      ...
      this.creditoCompraService
        .addCreditoAbonoFactura(this.factura_compra_id, this.form.value)
        .subscribe(
          (_) => {
            this.onSuccess(
              'Abono exitoso',
          ...
          (_) => {
            this.onSuccess(
              'Acción errónea',
              'Ocurrió un error al realizar el abono',
              TypeAlert.warning
            );
            this.btnDisabled = false;
          }
  ...
  getHistorial(page): void {
    ...
    this.creditoCompraService
      ...
      .subscribe((resp) => {
        ...
        this.pages = of(resp.data.abonos.links);
        this.loaded = true;
      });
  }
  ...
  delete(self, item) {
    self.creditoCompraService.deleteAbonoFactura(item).subscribe((_) => {
      self.onSuccess(
        'Abono eliminado exitosamente',
