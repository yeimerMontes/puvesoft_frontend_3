...
export class NotaDebitoElectronicaComponent implements OnInit {
  ...
  total() {
    ...
    for (let i = 0; i < f.length; i++) {
      ...
      if (
        ...
        formSubTotal =  Number(( Number(vlr_venta) * Number(can_dev) ) - vlr_venta_actual);
  ...
  devolver() {
    ...
    //   if (this.formDevolver.get('tipo_concepto_anulacion').value == 4) {
    //     formuDevolverValue.prds[i].valor_venta_movimiento = 0;
    //   } else {
    //     formuDevolverValue.prds[i].cantidad_movimiento = 0;
    //   }
