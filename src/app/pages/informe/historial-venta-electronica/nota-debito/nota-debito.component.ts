...
export class NotaDebitoElectronicaComponent implements OnInit {
  ...
  titleModule: string = 'Facturación electrónica - Nota débito';
  ...
  tipoConceptoById(id) {
    if (id){ 
      return this.tipoNotaDebitoFacturaElectronica.find(element => element.id == id).nombre
    }
    return '';
  }
  ...
  generateProductos() {
    ...
    this.formDevolver.patchValue({ tipo_nota: 2 }); // Nota débito
  ...
  private getProductosByIdFactura(facturaId) {
    ...
    this.notaFacturaElectronicaService
      .getEstadoFacturaElectronica(facturaId, 2)
      .subscribe((resp) => {
        ...
        if (!this._dataFactura.disabled_propina) {
          ...
          this.sumaPropina = +(this._dataFactura.propina);
        } else {
        ...
        if ((this._dataFactura.domicilio_id != null) && (!this._dataFactura.disabled_domicilio) ) {
          ...
          this.sumaDomicilio = +(resp.data.domicilio.valor);
        } else {
          this.formDevolver.patchValue({ domicilio_actual: 0 });
          this.sumaDomicilio = 0;
        }
    ...
    this.timeClear = setTimeout(() => {
      ...
      if (event.target.value != '' && (+event.target.value) < 0) {
        this.onSuccess(
          ...
          'La cantidad a agregar no puede ser menor a 0',
          TypeAlert.warning
        );
  ...
  total() {
    ...
    for (let i = 0; i < f.length; i++) {
      ...
      if (this.formDevolver.get('tipo_concepto_anulacion').value == 4) {
        ...
        this.formDevolver.get('tipo_concepto_anulacion').value == 2 ||
      ...
      if (
        ...
        formSubTotal =  Number(( Number(vlr_venta) * Number(can_dev) ) - total_actual);
        ...
        let nuevo_valor_venta_movimiento = valor_venta_movimiento - valor_venta_actual;
  ...
  valor_pagar_form(event, variable, variable2) {
    ...
    if (this.funcionesService.countDecimalPoints(val) === 0) {
      ...
      this.formDevolver.get(variable)!.setValue(+(val.replaceAll(',', '')+""));
    } else {
      this.formDevolver.get(variable)!.setValue(this.formatearNumberNoDecimal(+(val.replaceAll(',', '')+"")));
    }
    if (variable == 'propina_movimiento') {
      this.sumaPropina = (+this.formDevolver.get(variable2).value) + (+(val.replaceAll(',', '')+""));
    } else {
      this.sumaDomicilio = (+this.formDevolver.get(variable2).value) + (+(val.replaceAll(',', '')+""));
    }
  ...
  subTotalItem(item) {
    ...
    if (this.formDevolver.get('tipo_concepto_anulacion').value == 4) {
      ...
      item.get('total_movimiento')!.setValue(total-vlr_venta_actual);
      item.get('subtotal_movimiento')!.setValue(total-vlr_venta_actual);
      return total-vlr_venta_actual;
  ...
  previsualizar() {
    ...
    if (this.formDevolver.get('tipo_concepto_anulacion').value == 4) {
      if (!this.hasDevolucion) {
          this.onSuccess(
            'Debe ingresar al menos un movimiento débito',
            'Diligencie al menos un movimiento débito para poder crear la nota.',
            TypeAlert.warning
          );
          return;
        }
    ...
    if (
      this.formDevolver.get('tipo_concepto_anulacion').value != null
      ) {
  ...
  devolver() {
    ...
    if (this.formDevolver.get('tipo_concepto_anulacion').value == 4) {
      if (!this.hasDevolucion) {
        ...
        if (menor) {
          this.onSuccess(
            'Error en la cantidad a agregar',
            'Verifique las cantidades a agregar, estas no pueden ser menores a 1...',
            TypeAlert.warning
          );
          this.disabled = false;
          return;
    ...
    for (let i = 0; i < formuDevolverValue.prds.length; i++) {
      let valor_venta_actual = +(formuDevolverValue.prds[i].valor_venta_actual+"").replaceAll(',', '');
      formuDevolverValue.prds[i]['valor_venta_actual'] = valor_venta_actual;
      let valor_venta_movimiento = +(formuDevolverValue.prds[i].valor_venta_movimiento+"").replaceAll(',', '');
      formuDevolverValue.prds[i]['valor_venta_movimiento'] = valor_venta_movimiento;



      if ((valor_venta_movimiento-valor_venta_actual) < 0) {
        this.onSuccess(
          'Espere!',
          formuDevolverValue.prds[i]['nombre'] + ': el valor final no debe ser menor al precio inicial',
          TypeAlert.warning
        );
        this.disabled = false;
        return;
    ...
    for (let i = 0; i < formuDevolverValue.prds.length; i++) {
      ...
      if (this.formDevolver.get('tipo_concepto_anulacion').value == 4 && (+formuDevolverValue.prds[i].cantidad_movimiento) > 0) {
        arrProducts.push(formuDevolverValue.prds[i])
      } else if (
        (
          this.formDevolver.get('tipo_concepto_anulacion').value == 1 
        ...
        (formuDevolverValue.prds[i].valor_venta_movimiento - formuDevolverValue.prds[i].valor_venta_actual) > 0
      ) {
        
        formuDevolverValue.prds[i]['valor_venta_movimiento'] = (
          +(formuDevolverValue.prds[i]['valor_venta_movimiento']+"".replaceAll(',', ''))
        ) - (
          +(formuDevolverValue.prds[i]['valor_venta_actual']+"".replaceAll(',', ''))
        );
    ...
    formuDevolverValue['subtotal_movimiento_s_imp'] = (this.subTotal) - this.impuestoMovimiento;
  ...
  changePropina(event) {
    if (this.formDevolver.value.is_propina) {
      ...
      this.sumaPropina = this.formDevolver.value.propina_actual
    } else {
  ...
  changeDomicilio(event) {
    if (this.formDevolver.value.is_domicilio) {
      ...
      this.sumaDomicilio = this.formDevolver.value.domicilio_actual
    } else {
