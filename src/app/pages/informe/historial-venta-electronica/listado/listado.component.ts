...
export class ListadoElectronicoComponent implements OnInit {
  ...
  @ViewChild('staticModal6', { static: false }) childModal6?: ModalDirective;
  @ViewChild('staticModalPdf', { static: false })
  childModalPdf?: ModalDirective;

  titleModule: string = 'Historial de ventas - Facturas Electrónicas';
  headers: headersMasterInterface[] = headersHistorialVenta;

  disabled = false;
  disabledEnvioDian = false;
  disabledEnvioEmail = false;
  ...
  ngOnInit() {
    ...
    this.envioEmail = this.formBuilder.group({
      email: ['', []],
    });
  ...
  openModal(item, accion: number) {
    ...
    switch (accion) {
      ...
      case 5: //reenvio de factura a la Dian
        ...
        break;
        case 6: //reenvio de email
        this.idInvoice = item.id;
        this.envioEmail.setValue({ email: this.dataFactura.cliente_email});
        this.childModal6?.show();
        break;
      default:
        break;
    }
  ...
  ///reenvio de mail
  reenvioEmailElectronico() {
    this.disabledEnvioEmail = true;

    this.historialVentaService.reenviarEmailElectronico(this.idInvoice, this.envioEmail.get('email')?.value).subscribe(
      (resp) => {
        this.funcionService.onSuccess(resp.message, 'success', '¡OK!');
        ///en el caso que sea exitoso el envio, refrescamos la tabla

        this.disabledEnvioEmail = false;
        this.closeModal6();
      },
      (error) => {
        this.disabledEnvioEmail = false;
