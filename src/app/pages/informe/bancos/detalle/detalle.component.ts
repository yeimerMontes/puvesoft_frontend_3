...
export class DetalleComponent implements OnInit {
  ...
  titleModule: string = 'Historial de bancos';
  headers: headersMasterInterface[] = headersBancoSeguimiento;
  ...
  ngOnInit() {
    ...
    this.formAjuste = this.formBuilder.group({
      ...
      valor_original: ['', [Validators.required]],
  ...
  getTipoMovimientoBancos(): void {
    this.bancosServices.getTipoMovimientoBancos().subscribe((resp) => {
      this.tipoMovimientoBancoArr = resp;
    });
  }
  ...
  getBancos(page, perPage, typeReport: TypeReport = TypeReport.noReport) {
    ...
    this.bancosServices
      .getBancosSeguimientos(
        ...
        this.search.controls.tipo_movimiento.value,
        perPage,
        typeReport
      )
      .subscribe((resp) => {
  ...
  incremento() {
    this.formAjuste.controls['tipo_ajuste'].setValue('1');
  }

  disminucion() {
    this.formAjuste.controls['tipo_ajuste'].setValue('2');
  }
  ...
  cambiarMetodoPago() {
    let metodo_pago = this.formAjuste.controls['metodo_pago'].value;
    let found = this.findMetodoPago(metodo_pago);

    this.formAjuste.controls['valor_original'].setValue(
      this.formatearNumber(found.total)
    );
  }
  ...
  onSubmit() {
    if (this.formAjuste.invalid) {
      ...
      if (this.campoEsValido('metodo_pago')) {
        msg += '<li>El método de pago es obligatorio.</li>';
      }

      if (this.campoEsValido('tipo_ajuste')) {
        msg += '<li>El tipo de ajuste es obligatorio.</li>';
      }
    ...
    if (this.isActionAdd) {
      this.bancosServices.addAjusteBanco(this.formAjuste.value).subscribe(
        (resp) => {
