...
export class NotaAjustePosElectronicaComponent implements OnInit {
  ...
  titleModule: string = 'Documentos Pos electrónico - Notas de ajustes';
  ...
  generateProductos() {
    ...
    this.formDevolver.patchValue({ tipo_nota: 4 }); // Nota crédito
  ...
  private getProductosByIdFactura(facturaId) {
    ...
    this.notaFacturaElectronicaService
      .getEstadoFacturaElectronica(facturaId, 4)
      .subscribe((resp) => {
  ...
  regresar() {
    this.router.navigate(['/informe/historialventaposelectronico']);
  }
